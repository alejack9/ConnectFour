...
public class ChooseMatch {
	...
	private static Size chooseSize(RuleSet referee) {
		String line = Utils.doInput(in, out, "\nDo  you want to use default matchfield size 6x7? (y/n) ",
				x -> x.equals("y") || x.equals("n"), String::valueOf);
		if (line.equals("y"))
			return referee.getDefaultSize();
	...
	private static Integer chooseNumber(String name) {
		int num = Utils.doInput(in, out, "\nChoose the number of " + name + " (bigger than 1) : ", x -> x > 1,
				Integer::parseInt);
		return num;
	}
	...
	private static int chooseFirstPlayer() {
		int num = Utils.doInput(in, System.out, "\nChoose first Player: (1 for Player 1, 2 for Player 2)",
				x -> x == 1 || x == 2, Integer::parseInt);
		return num - 1;
	}
	...
	private static RuleSet chooseRuleset() {
		int num = Utils.doInput(in, System.out, requestP("RuleSet"), x -> x >= 0 && x < RuleSetType.values().length,
				Integer::parseInt);
	...
	private static Player choosePlayerType(int numplayer) {
		System.out.println("\nPlayer " + numplayer + " : ");
		int num = Utils.doInput(in, System.out, requestP("Player"), x -> x >= 0 && x < PlayerType.values().length,
				Integer::parseInt);
	 ...
	 * @return String that specifies available Player types or Ruleset types the
	 *         user can choose
	 */
